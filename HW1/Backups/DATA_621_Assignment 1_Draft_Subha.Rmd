...
The above table we see, model 4 and model 6 are looking the best fit. But if we look at p-value of all explanatory  variables is higer than 0.05.So, we will go with Model 6, which having high $R^2$ and $Adj R ^ 2$. Lower values of RMSE indicate better fit. RMSE is a good measure of how accurately the model predicts the response, and it is the most important criterion for fit if the main purpose of the model is prediction. 
...
prediction <- predict(model_6, MB_test, interval = "prediction")

DT::datatable(prediction)

summary(prediction)

MB_test$PRED_WINS <- round(predict(model_6,  newdata = MB_test),0)
