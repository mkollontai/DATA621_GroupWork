...
kable(crime_train_df[1:15,]) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```
...
kable(psych::describe(crime_train_df), digits = 3) %>% kable_styling(bootstrap_options = c("striped", "hover", "responsive"),latex_options="scale_down")
```
...
kable(cor_train, digits = 3) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```
...
kable(cor_train_target, digits = 3) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
kable(model1.data %>% top_n(3, .cooksd), digits = 1)  %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```
...
```{r,message=FALSE}
kable(model1.data %>% 
  filter(abs(.std.resid) > 3), digits = 1) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```
...
model3 <- glm(formula = target ~ indus + rm + lstat + medv, family = "binomial", data = df_train)
summary(model3)
```
...
metrics <- data.frame(round(Accuracy,4), round(CER,4), round(Precision,4), round(Sensitivity,4), round(Specificity,4), round(F1,4))
...
Actually_Positive <- c(TP,FN)
Actually_Negative <- c(FP,TN)
conf_mtx <- data.frame(Actually_Positive,Actually_Negative)
row.names(conf_mtx) <- c('Guess Positive', 'Guess Negative')
```
...
metrics
```
When based on the training data set, our metrics paint a fairly good picture. We can see that the accuracy, precision and specificity of our model are all just over 90%, with both the sensitivity and F1-score falling slightly short of that level. 
Finally, below is our prediction for the Evaluation dataset. 

```{r}
url_crime_eval_df<- 'https://raw.githubusercontent.com/ErindaB/data-621/main/crime-evaluation-data_modified.csv'
crime_eval_df <- read.csv(url_crime_eval_df, header = TRUE)
Final_Pred <- data.frame(predict(model2,crime_eval_df, type = 'response', se.fit = FALSE))
colnames(Final_Pred)[1] <- c('model')
Final_Pred$guess <- ifelse(Final_Pred$model > 0.5,1,0)
Final_Pred
```
...
```{r pie_compare, echo = FALSE}
Train_pie <- c(length(which(crime_train_df$target == 1)),length(which(crime_train_df$target == 0)))
Predict_pie <- c(length(which(Final_Pred$guess == 1)),length(which(Final_Pred$guess == 0)))
crim <- c('Higher Crime', 'Lower Crime')
par(mfrow=c(1,2))
train_lab <- paste(crim, round(Train_pie/sum(Train_pie)*100))
train_lab <- paste(train_lab,"%",sep='')
pie(Train_pie, labels = train_lab, main = 'Training Data', col=rainbow(length(crim)))
pred_lab <- paste(crim, round(Predict_pie/sum(Predict_pie)*100))
pred_lab <- paste(pred_lab,"%",sep='')
pie(Predict_pie, labels = pred_lab, main = 'Prediction', col=rainbow(length(crim)))
```
