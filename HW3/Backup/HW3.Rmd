...
We will start with a model containing  all un-transformed variables.

#### Model 1 - All Variables Untransformed

```{r,message=FALSE}
#build model 1 - all variables un-transformed
...
In this section we will select the variables with probability less than $.05$.

```{r}
#create model
model3 <- glm(formula = target ~ dis+rad+tax+ptratio+medv, family = binomial, data = df_train)
summary(model3)
```
...
We will be using the $compare_performance$ function from the $performance$ package to compare the 3 models we have so far. When take a look at the 3 models we can see that the $Model2_coll$ is a great fit as its AIC and BIC is lower than the other two. In this case we can probably conclude that $model2_coll$ the one with collinearity is the best performing model our of three we chose. 

```{r}
library(performance)
compare_performance(model1, model2_coll, model3, rank = 3)
```
